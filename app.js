...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // We start the timer immediately so it fades out no matter what happens with the DB.
    setTimeout(() => {
        const splash = document.getElementById('splash-screen');
        if(splash) {
            splash.classList.add('fade-out'); // Start fading
            setTimeout(() => {
                splash.style.display = 'none'; // Remove completely
    ...
    try {
        ...
        // If DB fails, we still want to see the app (even if empty)
...
async function exportAndClearData() {
    ...
    // --- NEW: Date Safety Check ---
    const today = new Date();
    const dayOfMonth = today.getDate();
    const isEndOfMonth = dayOfMonth >= 25; // Defines "End of Month" as 25th or later

    if (!isEndOfMonth) {
        const proceed = confirm(`⚠️ WAIT! It is only the ${dayOfMonth}th.\n\nAre you sure you want to close the monthly ledger? This will DELETE current data.`);
        if (!proceed) return;
    }
    // ------------------------------
...
// REPLACE your existing logCook function with this:
window.logCook = async function(recipeId) {
    const recipes = await db.getAll('recipes');
    const recipe = recipes.find(r => r.id === recipeId);
    if (!recipe) return;

    // 1. Identify what needs to be deducted
    const toDeduct = recipe.ingredients.filter(ing => ing.auto === true);

    // 2. Build the confirmation message
    let msg = `Log "${recipe.name}" as eaten?`;
    if (toDeduct.length > 0) {
        msg += `\n\n📉 Stock will be reduced for:\n` + toDeduct.map(i => `• ${i.name}: -${i.qty} ${i.unit}`).join('\n');
    } else {
        msg += `\n\n(No ingredients marked for auto-deduction)`;
    }
    ...
    if (toDeduct.length > 0) {
        ...
        for (const ing of toDeduct) {
            // Find the item in inventory by name (case-insensitive check is safer)
            const stockItem = inventory.find(i => i.name.toLowerCase() === ing.name.toLowerCase());
            
            if (stockItem) {
                stockItem.quantity = parseFloat(stockItem.quantity) - parseFloat(ing.qty);
                // Prevent negative stock
                if (stockItem.quantity < 0) stockItem.quantity = 0;
                // Round to 2 decimals to avoid "1.99999999" errors
                stockItem.quantity = Math.round(stockItem.quantity * 100) / 100;
                
                await db.update('inventory', stockItem);
            }
        }
    }

    // 4. Update History (Variety Score)
    recipe.lastCooked = new Date().toISOString();
    await db.update('recipes', recipe);
    
    alert("Meal logged & Inventory updated!");
    loadCookingView();
};
...
// REPLACE your existing addIngredientRow function with this:
window.addIngredientRow = function(data = {}) {
    const container = document.getElementById('ing-container');
    const div = document.createElement('div');
    div.className = 'ing-row';
    
    // Logic: If data.auto is true (or undefined/new row), default to CHECKED? 
    // Better safety: Default to FALSE for existing, let user check it. 
    // But for new rows, maybe you want it unchecked by default.
    const isChecked = data.auto ? 'checked' : '';

    div.innerHTML = `
        <div style="display:flex; flex-direction:column; align-items:center; margin-right:5px;">
            <label style="font-size:0.6rem; color:#888;">Auto</label>
            <input type="checkbox" class="ing-auto-check" ${isChecked} title="Auto-deduct from stock?">
        </div>
...
// REPLACE your existing saveRecipe function with this:
async function saveRecipe() {
    ...
    rows.forEach(row => {
        ...
        const autoCheck = row.querySelector('.ing-auto-check'); // <--- NEW
        ...
        if (select.value && qtyInput.value) {
            ingredients.push({
                ...
                auto: autoCheck.checked // <--- SAVING THE CHECKBOX STATE
            });
        }
    });
